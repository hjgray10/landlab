...
class BagnoldWindTransporter(Component):
    ...
    def convert_azimuth_to_cartesian(self):
        ...
        self.grid["node"]["wind__direction"] = np.abs(
            np.abs(self.grid["node"]["wind__direction"] - 360) + 90
        )
        self.grid["node"]["wind__direction"][
            np.where(self.grid["node"]["wind__direction"] > 359)
        ] -= 360

        ## then convert into radians
        self.grid["node"]["wind__direction"] = np.deg2rad(
            self.grid["node"]["wind__direction"]
        )
    ...
    def wind_sed_flux_calc(self):
        ...
        wind_avg_x_component_on_link = (
            np.sum(
                self.grid["node"]["wind__shear_velocity"][self.grid.nodes_at_link]
                * np.cos(self.grid["node"]["wind__direction"][self.grid.nodes_at_link]),
                axis=1,
            )
            / 2.0
        )

        wind_avg_y_component_on_link = (
            np.sum(
                self.grid["node"]["wind__shear_velocity"][self.grid.nodes_at_link]
                * np.sin(self.grid["node"]["wind__direction"][self.grid.nodes_at_link]),
                axis=1,
            )
            / 2.0
        )
        ...
        arctan_of_wind_x_y_components = np.arctan(
            wind_avg_y_component_on_link / wind_avg_x_component_on_link
        )
        ...
        quadrant_of_wind_arctan[
            np.logical_and(
                wind_avg_x_component_on_link > 0, wind_avg_y_component_on_link > 0
            )
        ] = 1
        ...
        wind_shear_on_the_links = np.sqrt(
            wind_avg_x_component_on_link ** 2.0 + wind_avg_y_component_on_link ** 2.0
        )
    ...
    def zero_soil_nodes(self, dt):
        ...
        link_angle_from_nodes_perspective = np.nan * np.zeros(
            np.shape(self.grid.links_at_node)
        )
        link_angle_from_nodes_perspective[
            self.grid.link_dirs_at_node == 1
        ] = incoming_link_angles
        link_angle_from_nodes_perspective[
            self.grid.link_dirs_at_node == -1
        ] = outgoing_link_angles

        wind_angles_reshape = np.nan * np.zeros(np.shape(self.grid.links_at_node))
        wind_angles_reshape[self.grid.links_at_node != -1] = self.grid["link"][
            "wind__angle_on_the_link"
        ][self.grid.links_at_node[self.grid.links_at_node != -1]]

        wind_incoming_or_outgoing = np.round(
            np.cos(wind_angles_reshape - link_angle_from_nodes_perspective)
        )
        ...
        outgoing_flux_per_node = self.grid["link"]["wind__sed_mass_flux"][
            self.grid.links_at_node
        ]  # get the sed flux at each link w.r.t. the node
        outgoing_flux_per_node[
            incoming_links_bool[0, :, :]
        ] = (
            0.0
        )  # for this array, set the incoming flux to zero as we don't care about it
        total_outgoing_flux_per_node = np.sum(
            np.abs(outgoing_flux_per_node), axis=1
        )  # figure out the outgoing flux

        zero_soil_danger_nodes = np.where(
            np.logical_and(
                self.grid["node"]["soil__depth"] <= total_outgoing_flux_per_node * dt,
                self.grid.status_at_node != CLOSED_BOUNDARY,
            )
        )  # these are the danger nodes

        # pick out the outgoing links on the danger nodes

        flux_on_links_at_danger_nodes = outgoing_flux_per_node[
            zero_soil_danger_nodes, :
        ]
        ...
        relative_flux_on_danger_links_at_danger_nodes = (
            flux_on_links_at_danger_nodes
            / np.max(np.abs(flux_on_links_at_danger_nodes[0, :, :]), axis=1)[:, None]
        )

        soil__depth_in_the_danger_nodes = self.grid["node"]["soil__depth"][
            zero_soil_danger_nodes
        ]
        new_outgoing_flux_on_links_out_of_danger_nodes = (
            relative_flux_on_danger_links_at_danger_nodes
            * (soil__depth_in_the_danger_nodes / dt)[:, None]
        )

        # now need to put it back into the larger flux array without screwing up the incoming fluxes

        replacement_link_index = self.grid.links_at_node[zero_soil_danger_nodes,][
            outgoing_links_bool[0, zero_soil_danger_nodes]
        ]
        replacement_link_flux = new_outgoing_flux_on_links_out_of_danger_nodes[
            outgoing_links_bool[0, zero_soil_danger_nodes]
        ]

        self.grid["link"]["wind__sed_mass_flux"][
            replacement_link_index
        ] = replacement_link_flux

    def bagnold_wind_transporter(self, dt):

        dhdt = -self.grid.calc_flux_div_at_node(
            self.grid["link"]["wind__sed_mass_flux"]
        )
